...
protocol INetworkManagerProtocol {
...
class WeatherNetworkManager : INetworkManagerProtocol {
    
    func fetchCurrentLocationWeather(lat: String, lon: String, completion: @escaping (WeatherModel) -> ()) {
        let API_URL = "http://api.openweathermap.org/data/2.5/weather?lat=\(lat)&lon=\(lon)&appid=1f467d888c5f11c0d870982aeb45b038"
    ...
    func fetchCurrentWeather(city: String, completion: @escaping (WeatherModel) -> ()) {
        ...
        let API_URL = "http://api.openweathermap.org/data/2.5/weather?q=\(formattedCity)&appid=1f467d888c5f11c0d870982aeb45b038"
    ...
    func fetchNextFiveWeatherForecast(city: String, completion: @escaping ([ForecastTemperature]) -> ()) {
        let formattedCity = city.replacingOccurrences(of: " ", with: "+")
        let API_URL = "http://api.openweathermap.org/data/2.5/forecast?q=\(formattedCity)&appid=1f467d888c5f11c0d870982aeb45b038"
             ...
             URLSession.shared.dataTask(with: urlRequest) { [weak self] (data, response, error) in
                guard let strongSelf = self else { return }
                 ...
                 do {
                    ...
                    var totalData = forecastWeather.list.count //должно быть 40
                    ...
                    for day in 0...forecastWeather.list.count - 1 {
                        ...
                        let weekdaycomponent = components.weekday! - 1  //значение от 0 до 6
                        
                        let f = DateFormatter()
                        let weekday = f.weekdaySymbols[weekdaycomponent] // 0 Sunday 6 - Saturday // string val (Mon/Tue/Wed...)
